        ...
        match(data[4]):
            case 0xE6:
                ...
                if(data[6] == 0x00):
                    ...
                    data = bytes([start,
                        0x1A, 0x00, # length = 26
                        0x04, 0x06, # preamble
                        0x5A, 0x00, 0x41, 0x00, 0x48, 0x00, 0x4C, 0x00, 0x47, 0x00, 0x48, 0x00, 0x36, 0x00, 0x43, 0x00, 0x00, 0x00, # serial number ZAHLGH6C in UTF-16LE format
                        end,
                        0x7C, 0x69 # checksum
                    ])
                if(data[6] == 0x01):
                    ...
                    data = bytes([start,
                        0x1E, 0x00, # length = 30
                        0x04, 0x06, # preamble
                        0x56, 0x00, 0x65, 0x00, 0x72, 0x00, 0x69, 0x00, 0x6F, 0x00, 0x20, 0x00, 0x46, 0x00, 0x6C, 0x00, 0x65, 0x00, 0x78, 0x00, 0x00, 0x00, # model name "Verio Flex" in UTF-16LE format
                        end,
                        0xE2, 0xEE # checksum
                    ])
            case 0x20:
                ...
                if(data[5] == 0x01):
                    ...
                    data = bytes([start,
                        0x08, 0x00, # length = 8
                        0x04, 0x06, # preamble
                        # empty payload as acknowledgement
                        end,
                        0x78, 0xC1 # checksum
                    ])
            case 0x27:
            ...
            case 0x31:
                ...
                data = bytes([start,
                    0x18, 0x00, # length = 24
                    0x04, 0x06, # preamble
                    0x0E, 0x00, # record number
                    0x00,
                    0x16, 0x00, # global record index = 22
                    0x29, 0xE3, 0xA7, 0x28, # timestamp 2021-08-12T13:48:25.000Z
                    0x32, 0x00, # glucose value = 50 mg/dL
                    0x00, 0x00, 0x93, 0x0B, 0x00, # flags
                    end,
                    0x61, 0x17 # checksum
                ])
            case _:
